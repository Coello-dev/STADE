...
class STD:
    ...
    def prune(
        ...
        W_metric = (self.layer.weight.data ** 2) * inp_var.reshape((1,-1))
        ...
        bias_data = torch.zeros((self.layer.weight.data.shape[0]), device=self.layer.weight.data.device ,dtype=self.layer.weight.data.dtype)
        bias_data += inp_mean@(self.layer.weight.data*W_mask.type(torch.float32)).transpose(0,1)
        if self.layer.bias is None:
            self.layer.bias = torch.nn.Parameter(bias_data, requires_grad=False)
        else:
            self.layer.bias = torch.nn.Parameter(self.layer.bias + bias_data, requires_grad=False)
